            ...
            return(
                <div className="swiper-slide" key={i}>
                    ...
                    <div className="service-grid-item__image">
                        ...
                        <Link to={`${process.env.PUBLIC_URL}/${lang}/${DPAC(val,"serviceUrl","")}`}>
                            <img src={`assets/img/service/${DPAC(val,"img","")}`} className="img-fluid" alt="" />
                        </Link>
                        </div>
                    </div>
                    <div className="service-grid-item__content">
                        <h3 className="title">
                        <Link to={`${process.env.PUBLIC_URL}/${lang}/${DPAC(val,"serviceUrl","")}`}>{DPAC(val,"serviceTitle","")}</Link>
                        </h3>
                        <p className="subtitle">{DPAC(val,"serviceExcerpt","")}</p>
                        <Link to={`${process.env.PUBLIC_URL}/${lang}${DPAC(val,"serviceUrl","")}`} className="see-more-link">SEE MORE</Link>
                    </div>
                    </div>
                </div>
            )
